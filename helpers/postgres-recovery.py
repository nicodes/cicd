...
PROJECTS = {'cazper', 'ormos', 'komizo', 'termcade', 'astry', 'fieldsofrevik', 'gdam'}
IMAGE_OWNERS = {
    'cazper': 'nicodes',
    'ormos': 'nicodes',
    'komizo': 'nicodes',
    'termcade': 'aviorstudio',
    'gdam': 'aviorstudio',
    'fieldsofrevik': 'aviorstudio',
    'astry': 'astrylogical',
}
...
def validate_manifest(value, expected_project=None, expected_revision=None):
    ...
    if (not isinstance(project, str) or project not in PROJECTS or not isinstance(revision, str)
    ...
    for component, image in images.items():
        prefix = f'ghcr.io/{IMAGE_OWNERS[project]}/{project}-{component}:'
