...
class PostgreSQLArchiveBoundaries(unittest.TestCase):
    ...
    def test_image_owner_follows_project_org_and_refuses_wrong_org(self):
        with tempfile.TemporaryDirectory() as directory:
            cazper = payload(Path(directory)/'cazper')
            pg.validate_manifest(cazper)
            self.assertEqual(cazper['images']['api']['reference'], f'ghcr.io/nicodes/cazper-api:{REVISION}')
            gdam = copy.deepcopy(cazper)
            gdam.update(project='gdam', database='gdam', roles=['gdam_owner', 'gdam_runtime'])
            for name in ['api', 'gate']:
                gdam['images'][name]['reference'] = f'ghcr.io/aviorstudio/gdam-{name}:{REVISION}'
            pg.validate_manifest(gdam)
            wrong_org = copy.deepcopy(gdam)
            wrong_org['images']['api']['reference'] = f'ghcr.io/nicodes/gdam-api:{REVISION}'
            with self.assertRaisesRegex(ValueError, 'application image'):
                pg.validate_manifest(wrong_org)
            unknown = copy.deepcopy(cazper)
    ...
    def host_local_manifest(self, root):
        ...
        manifest['images']['api']['reference'] = 'ghcr.io/nicodes/cazper-api:inapi-fold'
        manifest['images']['gate']['reference'] = 'ghcr.io/nicodes/cazper-gate:' + 'e'*40
        return manifest
    ...
    def test_host_local_transition_binds_each_component_tag_truthfully(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(ValueError, 'revision mismatch'):
                pg.validate_manifest(manifest, 'cazper', REVISION)

    def test_mixed_hex_rollout_binds_each_component(self):
        # termcade #181: the gate may roll on a different 40-hex tag than the
        # api; the api tag still anchors the single manifest revision.
        with tempfile.TemporaryDirectory() as directory:
            manifest = payload(Path(directory)/'payload')
            manifest['images']['gate']['reference'] = 'ghcr.io/nicodes/cazper-gate:' + 'f'*40
            pg.validate_manifest(manifest)
    ...
    def test_host_local_and_mixed_binding_stay_strict(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            refusals = [
                ...
                lambda m: m['images']['api'].update(reference='ghcr.io/attacker/cazper-api:inapi-fold'),
                lambda m: m['images']['api'].update(reference='ghcr.io/nicodes/ormos-api:inapi-fold'),
                lambda m: m['images']['api'].update(reference='ghcr.io/nicodes/cazper-gate:inapi-fold'),
                # The api tag must anchor the recorded host-local revision.
                lambda m: m['images']['api'].update(reference='ghcr.io/nicodes/cazper-api:other-fold'),
                # Tags stay within the docker tag charset even in transition mode.
                lambda m: m['images']['gate'].update(reference='ghcr.io/nicodes/cazper-gate:-bad'),
                lambda m: m['images']['gate'].update(reference='ghcr.io/nicodes/cazper-gate:has/slash'),
            ...
            release['images']['api']['reference'] = f'ghcr.io/nicodes/cazper-api:{REVISION}'
            release['images']['gate']['reference'] = 'ghcr.io/nicodes/cazper-gate:latest'
            with self.assertRaisesRegex(ValueError, 'application image'):
                pg.validate_manifest(release)
            # The api tag must equal the hex revision, not just any hex tag.
            release['images']['gate']['reference'] = f'ghcr.io/nicodes/cazper-gate:{REVISION}'
            release['images']['api']['reference'] = 'ghcr.io/nicodes/cazper-api:' + 'b'*40
            with self.assertRaisesRegex(ValueError, 'application image'):
                pg.validate_manifest(release)
